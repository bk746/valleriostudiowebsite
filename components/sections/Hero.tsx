...
export default function Hero() {
  return (
    <section
      className={`${bebas.className} sticky top-0 z-0 flex h-[100svh] flex-col bg-[#FFFFFF] px-5 pb-6 pt-0 sm:px-8 md:px-12`}
    >
      <div className="flex flex-1 flex-col items-center justify-center gap-5 -translate-y-6 sm:gap-7 sm:-translate-y-12">
        <h1
          className="text-center text-[clamp(3rem,17vw,15rem)] font-normal leading-[0.92] text-[#1D1D1F]"
          aria-label={HERO_TITLE}
        >
          <HeroTitleLetters text={HERO_TITLE} />
        </h1>
        <p className="text-center text-[clamp(0.7rem,1.2vw,1.25rem)] font-normal leading-tight tracking-[0.24em] text-[#1D1D1F] sm:tracking-[0.28em]">
          ON CONÇOIT DES SITES QUI CONVERTISSENT
        </p>
      </div>

      <div className="flex items-end justify-between gap-3 text-[0.7rem] text-[#1D1D1F] sm:gap-4 sm:text-xs md:text-sm">
